...
class _Action extends \IPS\Node\Model
{
    ...
    public function init()
    {
        ...
        if (class_exists($extClass)) {
            if (isset(static::$actionsCache[$this->app][$this->class])) {
                ...
                $ext = new $extClass;
                $actions = static::$actionsCache[$this->app][$this->class] = (method_exists(
                    $ext,
                    'actions'
                ) ? $ext->actions() : []);
            }
    ...
    public function form(&$form)
    {
        ...
        $scheduling_toggles = [
            ...
            3 => ['action_schedule_date', 'action_schedule_key'],
            4 => ['action_schedule_customcode', 'action_schedule_key'],
        ];

        $form->add(
            new \IPS\Helpers\Form\Radio(
                'action_schedule_mode',
                is_numeric($this->schedule_mode) ? $this->schedule_mode : 1,
                true,
                ['options' => $scheduling_options, 'toggles' => $scheduling_toggles]
            ),
            'operation_title'
        );
        $form->add(
        ...
        $form->add(
            new \IPS\Helpers\Form\Date(
                ...
                \IPS\DateTime::ts($this->schedule_date),
                false,
                ['time' => true],
                null,
                null,
        ...
        try {
            $lang->words['action_schedule_customcode_desc'] = $lang->get('phpcode_desc') . "<br><br>" . $lang->get(
                    'phpcode_desc_details'
                ) . $lang->get('action_schedule_customcode__desc') . "<br>" . $lang->get(
                    'phpcode_desc_details_vars'
                ) . \IPS\rules\Application::eventArgInfo($event);
        } catch (\Exception $e) {
        }

        $form->add(
            new \IPS\Helpers\Form\YesNo('action_enable_recursion', $this->enable_recursion, false),
            'operation_title'
        );
    ...
    public function saveForm($values)
    {
        ...
        $values = \IPS\rules\Application::opformSave(
            $this,
            'actions',
            $values,
            [
    ...
    public function invoke()
    {
        if (!$this->locked or $this->enable_recursion) {
            ...
            try {
                call_user_func_array('\IPS\rules\Application::opInvoke', [$this, 'actions', func_get_args()]);
            } catch (\Exception $e) {
                $this->locked = false;
                throw $e;
            }
            ...
            if ($rule = $this->rule() and $rule->debug) {
                \IPS\rules\Application::rulesLog(
                    ...
                    $this,
                    '--',
                    'Action recursion (not evaluated)'
                );
            }
        }
    }
