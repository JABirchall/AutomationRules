    ...
    public function save()
    {
        ...
        if (!$this->_new) {
             ...
             */
                ...
                if ($this->originalData['type'] != $this->type or $this->originalData['varname'] != $this->varname) {
                    \IPS\Db::i()->changeColumn(
                        $this->databaseTable(),
                        'data_' . $this->originalData['varname'],
                        \IPS\rules\Data::columnDefinition($this->type, $type_class, $this->varname)
                    );
                }
            }
        }
         ...
         */
        if (!\IPS\Db::i()->checkForColumn(
            \IPS\rules\Log\Custom::getTableName($this->parent()->class),
            'data_' . $this->varname
        )) {
            \IPS\Db::i()->addColumn(
                $this->databaseTable(),
