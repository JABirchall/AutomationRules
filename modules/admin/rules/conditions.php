...
class _conditions extends \IPS\Node\Controller
{
                    ...
                    if (\IPS\Request::i()->rule) {
                        ...
                        \IPS\Output::i()->redirect(
                            \IPS\Http\Url::internal(
                                "app=rules&module=rules&controller=rules&do=form&id={$rule_id}&tab=conditions"
                            )
                        );
                    }
                }
        ...
        if (isset ($this->rule)) {
            if (!\IPS\Request::i()->isAjax()) {
                \IPS\Output::i()->output .= "
					...
					<a href='" . \IPS\Http\Url::internal(
                        "app=rules&module=rules&controller=rules&do=form&id={$this->rule->id}&tab=conditions"
                    ) . "' class='ipsButton ipsButton_small ipsButton_positive'><i class='fa fa-arrow-left'></i> &nbsp;Back To Rule</a>";
            }
        }
    ...
    public function _getRoots()
    {
        ...
        $rows = [];
        foreach ($nodeClass::roots(null, null, [['condition_rule_id=?', $this->rule->id]]) as $node) {
            ...
            return [
                'add' => [
                    ...
                    'data' => ($nodeClass::$modalForms ? [
                        'ipsDialog' => '',
                        'ipsDialog-title' => \IPS\Member::loggedIn()->language()->addToStack('rules_add_condition'),
                    ] : []),
                ],
            ];
        }
    ...
    protected function _afterSave(\IPS\Node\Model $old = null, \IPS\Node\Model $new, $lastUsedTab = false)
    {
        if (\IPS\Request::i()->isAjax()) {
            ...
            if ($old == null) {
                \IPS\Output::i()->redirect(
                    \IPS\Http\Url::internal(
                        "app=rules&module=rules&controller=conditions&do=form&id={$new->id}&rule={$new->rule_id}"
                    )
                );
            } else {
                \IPS\Output::i()->redirect(
                    \IPS\Http\Url::internal(
                        "app=rules&module=rules&controller=rules&do=form&id={$new->rule_id}&tab=conditions"
                    ),
                    'saved'
                );
            }
