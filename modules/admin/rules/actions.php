                    ...
                    if (\IPS\Request::i()->rule) {
                        ...
                        \IPS\Output::i()->redirect(
                            \IPS\Http\Url::internal(
                                "app=rules&module=rules&controller=rules&do=form&id={$rule_id}&tab=actions"
                            )
                        );
                    }
                }
    ...
    protected function _afterSave(\IPS\Node\Model $old = null, \IPS\Node\Model $new, $lastUsedTab = false)
    {
        if (\IPS\Request::i()->isAjax()) {
            ...
            if ($old == null) {
                \IPS\Output::i()->redirect(
                    \IPS\Http\Url::internal(
                        "app=rules&module=rules&controller=actions&do=form&id={$new->id}&rule={$new->rule_id}"
                    )
                );
            } else {
                \IPS\Output::i()->redirect(
                    \IPS\Http\Url::internal(
                        "app=rules&module=rules&controller=rules&do=form&id={$new->rule_id}&tab=actions"
                    ),
                    'saved'
                );
            }
