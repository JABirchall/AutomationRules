...
class _rulesets extends \IPS\Node\Controller
{
    ...
    protected function form()
    {
        ...
        if ($rule and $rule->parent()) {
            ...
            if (\IPS\Request::i()->parent and \IPS\Request::i()->subnode == 0) {
                /**
    ...
    public function _getRow($id, $root = false, $noSort = false)
    {
        ...
        if (isset(\IPS\Request::i()->searchResult) and isset($buttons['edit'])) {
            $buttons['edit']['link'] = $buttons['edit']['link']->setQueryString(
                'searchResult',
        ...
        if ($node instanceof \IPS\rules\Rule) {
            ...
            if ($node instanceof \IPS\rules\Rule\Ruleset) {
                ...
                if ($old == null) {
                    \IPS\Output::i()->redirect(
                        \IPS\Http\Url::internal(
                            "app=rules&module=rules&controller=rulesets&subnode=1&do=form&id={$new->id}&tab=conditions"
                        )
                    );
                }
            }
    ...
    protected function debugEnable()
    {
        ...
        \IPS\Output::i()->redirect(
            \IPS\Http\Url::internal("app=rules&module=rules&controller=rulesets"),
            'Debugging Enabled'
        );
    }
    ...
    public function import()
    {
        ...
        if ($values = $form->values()) {
            ...
            \IPS\Output::i()->redirect(
                \IPS\Http\Url::internal(
                    'app=rules&module=rules&controller=rulesets&do=doImport&file=' . urlencode(
                        $tempFile
                    ) . '&key=' . md5_file($tempFile) . (isset(\IPS\Request::i()->id) ? '&id=' . \IPS\Request::i(
                        )->id : '')
                )
            );
        }
    ...
    {
        if (!file_exists(urldecode(\IPS\Request::i()->file)) or md5_file(
                urldecode(\IPS\Request::i()->file)
            ) != \IPS\Request::i()->key) {
        ...
        if (\IPS\Application::load('rules')->isProtected()) {
            if (\IPS\Db::i()->select('COUNT(*)', 'rules_rules')->first() + ($importCount = $this->_importRulesCount(
                    $import
                )) > \IPS\rules\Secure\RULELIMIT) {
                \IPS\Output::i()->error(
                    'Lite version restricted to a maximum of ' . \IPS\rules\Secure\RULELIMIT . ' rules. This import contains ' . $importCount . ' rules.',
                    'RULES',
                    200,
                    ''
                );
